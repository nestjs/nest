...
class ClientProxy {
    ...
    connect$(instance, errorEvent = constants_1.ERROR_EVENT, connectEvent = constants_1.CONNECT_EVENT) {
        const error$ = rxjs_2.fromEvent(instance, errorEvent).pipe(operators_1.map(err => {
            throw err;
        }));
        const connect$ = rxjs_2.fromEvent(instance, connectEvent);
        return rxjs_2.merge(error$, connect$).pipe(operators_1.take(1));
    }
