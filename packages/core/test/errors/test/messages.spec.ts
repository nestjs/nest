...
describe('UnknownDependenciesMessage', () => {
  ...
  it('should display class', () => {
    ...
    const expectedResult = stringCleaner(`
    ...
    const actualMessage = stringCleaner(
      new UnknownDependenciesException('CatService', { index, dependencies: ['', CatService] }).message
    );
  ...
  it('should display the provide token', () => {
    ...
    const actualMessage = stringCleaner(
      new UnknownDependenciesException('CatService', { index, dependencies: ['', 'MY_TOKEN'] }).message
    );
  ...
  it('should display the function name', () => {
    ...
    const actualMessage = stringCleaner(
      new UnknownDependenciesException('CatService', { index, dependencies: ['', CatFunction] }).message
    );
    expect(actualMessage).to.equal(expectedResult);
  });
  it('should use "+" if unknown dependency name', () => {
    ...
    const actualMessage = stringCleaner(
      new UnknownDependenciesException('CatService', { index, dependencies: ['', undefined] }).message
    );
  ...
  it('should display the module name', () => {
    ...
    const expectedResult = stringCleaner(`
    Nest can't resolve dependencies of the CatService (?, MY_TOKEN).
    Please make sure that the argument dependency at index [0] is available in the TestModule context.
    ...
    const actualMessage = stringCleaner(
      new UnknownDependenciesException('CatService', { index, dependencies: ['', 'MY_TOKEN'] }, myModule as Module).message
    );
  ...
  it('should display the symbol name of the provider', () => {
    const expectedResult = stringCleaner(`
    Nest can't resolve dependencies of the Symbol(CatProvider) (?).
    Please make sure that the argument dependency at index [0] is available in the current context.
    ...
    const actualMessage = stringCleaner(
      new UnknownDependenciesException(Symbol('CatProvider'), { index, dependencies: [''] }).message
    );
  ...
  it('should display the symbol dependency of the provider', () => {
    const expectedResult = stringCleaner(`
    Nest can't resolve dependencies of the CatProvider (?, Symbol(DogProvider)).
    Please make sure that the argument dependency at index [0] is available in the current context.
    ...
    const actualMessage = stringCleaner(
      new UnknownDependenciesException('CatProvider', { index, dependencies: ['', Symbol('DogProvider')] }).message
    );
