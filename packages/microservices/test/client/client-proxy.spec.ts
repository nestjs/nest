...
describe('ClientProxy', function() {
  ...
  describe('addMessageRequest', () => {
    it(`should add request map`, () => {
      const requestPattern = 'pattern.request';
      const replyPattern = 'pattern.request.reply';

      sinon.stub(client as any, 'requestMap').value({ set() {} });

      const clientNormalizePatternSpy = sinon.spy(client as any, 'normalizePattern');

      const requestMapSetSpy = sinon.spy(
        (client as any).requestMap,
        'set',
      );

      client.addMessageRequest(requestPattern, replyPattern);

      expect(clientNormalizePatternSpy.calledTwice).to.be.true;
      expect(requestMapSetSpy.called).to.be.true;
      expect(requestMapSetSpy.args[0][0]).to.be.equal(requestPattern);
      expect(requestMapSetSpy.args[0][1]).to.be.equal(replyPattern);
    });
  });
  ...
  describe('getReplyPattern', () => {
    ...
    it('should return reply pattern', () => {
      client.addMessageRequest(requestPattern, replyPattern);

      const requestMapGetSpy = sinon.spy((client as any).requestMap, 'get');

      const value = (client as any).getReplyPattern(requestPattern, affix);

      expect(requestMapGetSpy.called).to.be.true;
      expect(requestMapGetSpy.args[0][0]).to.be.equal(requestPattern);
      expect(value).to.be.equal(replyPattern);
    });

    it('should return reply pattern without explicit reply pattern', () => {
      client.addMessageRequest(requestPattern);
