...
export interface RouteParamsMetadata {
  [prop: number]: {index: number; data?: ParamData;};
}

const assignMetadata =
    (args: RouteParamsMetadata, paramtype: RouteParamtypes, index: number,
     data?: ParamData, ...pipes: PipeTransform<any>[]) => ({
      ...args,
      [`${paramtype}:${index}`] : {
        index,
        data,
...
const createPipesRouteParamDecorator = (paramtype: RouteParamtypes) =>
    (data?, ...pipes: PipeTransform<any>[]): ParameterDecorator => (target, key,
                                                                    index) => {
      const args = Reflect.getMetadata(ROUTE_ARGS_METADATA, target, key) || {};
...
export function Query(property?: string|PipeTransform<any>,
...
export function Body(property?: string|PipeTransform<any>,
                     ...pipes: PipeTransform<any>[]) {
...
export function Param(property?: string|PipeTransform<any>,
                      ...pipes: PipeTransform<any>[]) {
