...
export interface INestApplication {
   ...
   *
   * @param  {string} prefix The prefix for the every HTTP route path (for
   * example `/v1/api`)
   * @returns void
   */
  ...
  /**
   * Connects microservice to the NestApplication instance. It transforms
   * application to the hybrid instance.
   *
  ...
  /**
   * Setups pipes as a global pipes (will be used within every HTTP route
   * handler)
   *
  ...
  /**
   * Setups guards as a global guards (will be used within every HTTP route
   * handler)
   *
  ...
  /**
   * Terminates the application (both NestApplication, Web Socket Gateways and
