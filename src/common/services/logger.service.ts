...
export class Logger {
  ...
  public static setMode(mode: NestEnvironment) { this.contextEnv = mode; }

  public log(message: string) { this.printMessage(message, clc.green); }
  ...
  public warn(message: string) { this.printMessage(message, clc.yellow); }

  private printMessage(message: string, color: (msg: string) => string) {
    if (Logger.contextEnv === NestEnvironment.TEST)
      return;
  ...
  private printStackTrace(trace: string) {
    if (Logger.contextEnv === NestEnvironment.TEST || !trace)
      return;
