...
describe('ClientTCP', () => {
  ...
  beforeEach(() => {
    socket = {
      ...
      on : sinon.stub().callsFake((event, callback) => event !== 'error' &&
                                                       event !== 'close' &&
                                                       callback({})),
      end : sinon.spy(),
    };
    createSocketStub =
        sinon.stub(client, 'createSocket').callsFake(() => socket);
  });
  afterEach(() => { createSocketStub.restore(); });
  describe('sendSingleMessage', () => {
    let msg;
    beforeEach(() => { msg = {test : 3}; });
  ...
  describe('handleResponse', () => {
    ...
    describe('when disposed', () => {
      ...
      it('should end server', () => { expect(socket.end.called).to.be.true; });
  ...
  describe('close', () => {
    ...
    it('should end() socket', () => { expect(socket.end.called).to.be.true; });
