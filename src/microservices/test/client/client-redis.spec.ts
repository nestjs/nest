...
describe('ClientRedis', () => {
  ...
  describe('sendSingleMessage', () => {
    const pattern = 'test';
    const msg = {pattern};
    let subscribeSpy: sinon.SinonSpy, publishSpy: sinon.SinonSpy,
        onSpy: sinon.SinonSpy, removeListenerSpy: sinon.SinonSpy,
        unsubscribeSpy: sinon.SinonSpy, initSpy: sinon.SinonSpy, sub, pub;
    ...
    it('should publish stringified message to acknowledge pattern name', () => {
      client['sendSingleMessage'](msg, () => {});
      expect(publishSpy.calledWith(`"${pattern}"_ack`, JSON.stringify(msg)))
          .to.be.true;
    });
