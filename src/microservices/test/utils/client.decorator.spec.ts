...
describe('@Client', () => {
  ...
  class TestComponent {
    @Client(pattern as any) public static instance;
  }
  it(`should enhance property with metadata`, () => {
    const isClient =
        Reflect.getOwnMetadata(CLIENT_METADATA, TestComponent, 'instance');
    const config = Reflect.getOwnMetadata(CLIENT_CONFIGURATION_METADATA,
