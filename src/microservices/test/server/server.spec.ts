...
describe('Server', () => {
  ...
  describe('send', () => {
    ...
    beforeEach(() => { stream$ = Observable.of('test'); });
    describe('when stream', () => {
      beforeEach(() => { sendSpy = sinon.spy(); });
      describe('throws exception', () => {
        beforeEach(() => { server.send(Observable.throw('test'), sendSpy); });
        it('should send error', () => {
          expect(sendSpy.calledWith({err : 'test', response : null}))
              .to.be.true;
        });
      ...
      describe('emits response', () => {
        beforeEach(() => { server.send(stream$, sendSpy); });
        it('should send response', () => {
          expect(sendSpy.calledWith({err : null, response : 'test'}))
              .to.be.true;
        });
        it('should send "complete" event',
           () => { expect(sendSpy.calledWith({disposed : true})).to.be.true; });
    ...
    describe('when resultOrDeffered', () => {
      describe('is Promise', () => {
        it('should returns Observable', async () => {
          const value = 100;
          expect(await server.transformToObservable(Promise.resolve(value))
                     .toPromise())
              .to.be.eq(100);
        });
      });
      describe('is Observable', () => {
        it('should returns Observable', async () => {
          const value = 100;
          expect(await server.transformToObservable(Observable.of(value))
                     .toPromise())
              .to.be.eq(100);
        });
      });
      describe('is value', () => {
        it('should returns Observable', async () => {
          const value = 100;
          expect(await server.transformToObservable(value).toPromise())
              .to.be.eq(100);
        });
      });
    });
