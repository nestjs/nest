...
export abstract class Server {
  ...
  public getHandlers(): MessageHandlers { return this.messageHandlers; }

  public add(pattern, callback: (data) => Promise<Observable<any>>) {
    this.messageHandlers[JSON.stringify(pattern)] = callback;
  }

  public send(stream$: Observable<any>,
              respond: (data: MicroserviceResponse) => void): Subscription {
    return stream$
        .catch((err) => {
  ...
  protected handleError(error: string) { this.logger.error(error); }
}