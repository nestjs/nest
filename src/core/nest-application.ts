...
import {
  MicroserviceConfiguration
    ...
    '@nestjs/common/interfaces/microservices/microservice-configuration.interface';
...
import {
  MicroservicesPackageNotFoundException
} from './errors/exceptions/microservices-package-not-found.exception';
...
export class NestApplication implements INestApplication {
  ...
  public connectMicroservice(config: MicroserviceConfiguration):
      INestMicroservice {
    if (!NestMicroservice) {
  ...
  public getMicroservices(): INestMicroservice[] { return this.microservices; }
  ...
  public use(requestHandler) { this.express.use(requestHandler); }
  ...
  private listenToPromise(microservice: INestMicroservice) {
    return new Promise(
        async (resolve, reject) => { await microservice.listen(resolve); });
  }

  private callInitHook() {
    const modules = this.container.getModules();
    modules.forEach((module) => { this.callModuleInitHook(module); });
  }
  ...
  private callDestroyHook() {
    const modules = this.container.getModules();
    modules.forEach((module) => { this.callModuleDestroyHook(module); });
  }
