...
describe('Injector', () => {
  ...
  describe('loadInstance', () => {
    ...
    beforeEach(() => {
      ...
      moduleDeps.components.set('MainTestResolved',
                                {...mainTest, isResolved : true});
    });

    it('should create an instance of component with proper dependencies',
       async () => {
         await injector.loadInstance(mainTest, moduleDeps.components,
                                     moduleDeps);
         const {instance} =
             moduleDeps.components.get('MainTest') as InstanceWrapper<MainTest>;
    ...
    it('should throw RuntimeException when type is not stored in collection',
       () => {
         return expect(
                    injector.loadInstance({} as any, moduleDeps.components,
                                          moduleDeps),
                    )
  ...
  describe('loadInstanceOfMiddleware', () => {
    ...
    it('should call "resolveConstructorParams" when instance is not resolved',
       () => {
         ...
         injector.loadInstanceOfMiddleware({metatype : {name : ''}} as any,
                                           collection as any, null);
  ...
  describe('loadInstanceOfRoute', () => {
    ...
    it('should call "loadInstance" with expected arguments', async () => {
      ...
      expect(loadInstance.calledWith(wrapper, module.routes, module))
          .to.be.true;
    });
  });
  ...
  describe('loadInstanceOfInjectable', () => {
    ...
    it('should call "loadInstance" with expected arguments', async () => {
      ...
      expect(loadInstance.calledWith(wrapper, module.injectables, module))
          .to.be.true;
    });
  });
  ...
  describe('resolveFactoryInstance', () => {
    it('should resolve deffered value', async () => {
      const wrapper = {test : 'test'};
      const result =
          await injector.resolveFactoryInstance(Promise.resolve(wrapper));
  ...
  describe('scanForComponent', () => {
    ...
    beforeEach(() => {
      ...
      (injector as any).scanForComponentInRelatedModules =
    ...
    it('should return object from collection if exists', async () => {
      ...
      const result = await injector.scanForComponent(
          collection as any, metatype.name, null, metatype);
    ...
    it('should call "scanForComponentInRelatedModules" when object is not in collection',
       async () => {
         ...
         await injector.scanForComponent(collection as any, metatype.name, null,
                                         metatype);
    ...
    it('should throw "UnknownDependenciesException" when instanceWrapper is null and "exports" collection does not contain token',
       () => {
         ...
         expect(
             injector.scanForComponent(collection as any, metatype.name,
                                       module as any, {metatype}),
             )
             .to.eventually.be.rejected;
       });
  ...
  describe('scanForComponentInRelatedModules', () => {
    ...
    it('should return null when related modules do not have appropriate component',
       () => {
         ...
         expect(
             injector.scanForComponentInRelatedModules(module as any,
                                                       metatype as any, []),
             )
    ...
    it('should call "loadInstanceOfComponent" when component is not resolved',
       async () => {
         ...
         await injector.scanForComponentInRelatedModules(module as any,
                                                         metatype as any, []);
  ...
  describe('scanForComponentInScopes', () => {
    it('should returns null when component is not available in any scope',
       () => {
         expect(injector.scanForComponentInScopes([], '', {}))
    ...
    it('should returns wrapper when component is available in any scope',
       () => {
         ...
         expect(injector.scanForComponentInScopes([ {} ] as any, '', {}))
  ...
  describe('scanForComponentInScope', () => {
    it('should returns null when scope throws exception', () => {
      ...
      expect(injector.scanForComponentInScope({} as any, '', {}))
          .to.eventually.be.null;
    });

    it('should rethrow UndefinedDependencyException', () => {
      sinon.stub(injector, 'scanForComponent').throws(
          new UndefinedDependencyException('type'));
      expect(injector.scanForComponentInScope({} as any, '', {}))
          .to.eventually.throw();
    });

    describe(
        'when instanceWrapper is not resolved and does not have forward ref',
        () => {
          it('should call loadInstanceOfComponent', async () => {
            const loadStub = sinon.stub(injector, 'loadInstanceOfComponent')
                                 .callsFake(() => null);
            sinon.stub(injector, 'scanForComponent').returns({
              isResolved : false
            });

            await injector.scanForComponentInScope([] as any, 'name',
                                                   {} as any);
            expect(loadStub.called).to.be.true;
          });
          it('should not call loadInstanceOfComponent (isResolved)',
             async () => {
               const loadStub = sinon.stub(injector, 'loadInstanceOfComponent')
                                    .callsFake(() => null);
               sinon.stub(injector, 'scanForComponent').returns({
          ...
          it('should not call loadInstanceOfComponent (forwardRef)',
             async () => {
               const loadStub = sinon.stub(injector, 'loadInstanceOfComponent')
                                    .callsFake(() => null);
               sinon.stub(injector, 'scanForComponent')
  ...
  describe('resolveParamToken', () => {
    ...
    describe('when "forwardRef" property is not nil', () => {
      ...
      it('return forwardRef() result', () => {
        expect(injector.resolveParamToken(wrapper, param))
            .to.be.eql(forwardRef);
      });
