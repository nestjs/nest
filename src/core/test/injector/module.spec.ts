...
} from '../../../common/utils/decorators/module.decorator';
...
describe('Module', () => {
  ...
  beforeEach(() => { module = new Module(TestModule as any, []); });
  ...
  describe('addCustomFactory', () => {
    ...
    const inject = [ 1, 2, 3 ];
    const component =
        {provide : type, useFactory : type, name : 'test', inject};
  ...
  describe('when exported component is custom provided', () => {
    it('should call `addCustomExportedComponent`', () => {
      const addCustomExportedComponentSpy =
          sinon.spy(module, 'addCustomExportedComponent');
