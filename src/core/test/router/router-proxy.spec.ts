  ...
  describe('createProxy', () => {
    ...
    it('should method encapsulate callback passed as argument', () => {
      ...
      const proxy = routerProxy.createProxy(
          (req, res, next) => { throw new HttpException('test', 500); },
          handler);
      proxy(null, null, null);
      expectation.verify();
    });

    it('should method encapsulate async callback passed as argument',
       (done) => {
         const expectation = handlerMock.expects('next').once();
         const proxy = routerProxy.createProxy(
             async (req, res,
                    next) => { throw new HttpException('test', 500); },
             handler);
         proxy(null, null, null);
  ...
  describe('createExceptionLayerProxy', () => {
    ...
    it('should method encapsulate callback passed as argument', () => {
      ...
      const proxy = routerProxy.createExceptionLayerProxy(
          (err, req, res, next) => { throw new HttpException('test', 500); },
          handler);
