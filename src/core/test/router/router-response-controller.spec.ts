...
describe('RouterResponseController', () => {
    ...
    describe('apply', () => {
        let response: { send: sinon.SinonSpy, status?: sinon.SinonSpy, json: sinon.SinonSpy };
        beforeEach(() => {
        ...
        describe('when resultOrDeffered', () => {
            describe('is Promise', () => {
                it('should returns Promise', async () => {
                    const value = 100;
                    expect(await routerResponseController.transformToResult(Promise.resolve(value))).to.be.eq(100);
                });
            });
            ...
            describe('is Observable', () => {
                it('should returns Promise', async () => {
                    const value = 100;
                    expect(await routerResponseController.transformToResult(Observable.of(value))).to.be.eq(100);
                });
            });
