...
describe('RouterExecutionContext', () => {
  ...
  describe('create', () => {
    describe('when callback metadata is not undefined', () => {
      ...
      beforeEach(() => {
        ...
        exchangeKeysForValuesSpy =
            sinon.spy(contextCreator, 'exchangeKeysForValues');
      });
      ...
      describe('returns proxy function', () => {
        ...
        beforeEach(() => {
          instance = {foo : 'bar'};
          proxyContext =
              contextCreator.create(instance, callback as any, '', '', 0);
        });
  ...
  describe('reflectCallbackMetadata', () => {
    ...
    it('should returns ROUTE_ARGS_METADATA callback metadata', () => {
      ...
      const metadata =
          contextCreator.reflectCallbackMetadata(instance, 'callback');
  ...
  describe('getParamValue', () => {
    ...
    beforeEach(() => { consumerApplySpy = sinon.spy(consumer, 'apply'); });
    describe('when paramtype is query, body or param', () => {
      it('should call "consumer.apply" with expected arguments', () => {
        contextCreator.getParamValue(
            value, {metatype, type : RouteParamtypes.QUERY, data : null},
            transforms);
        expect(consumerApplySpy.calledWith(
                   value, {metatype, type : RouteParamtypes.QUERY, data : null},
                   transforms))
            .to.be.true;

        contextCreator.getParamValue(
            value, {metatype, type : RouteParamtypes.BODY, data : null},
            transforms);
        expect(consumerApplySpy.calledWith(
                   value, {metatype, type : RouteParamtypes.BODY, data : null},
                   transforms))
        ...
        contextCreator.getParamValue(
            value, {metatype, type : RouteParamtypes.PARAM, data : null},
            transforms);
        expect(consumerApplySpy.calledWith(
                   value, {metatype, type : RouteParamtypes.PARAM, data : null},
                   transforms))
    ...
    describe('when paramtype is not query, body and param', () => {
      it('should not call "consumer.apply"', () => {
        contextCreator.getParamValue(
            value, {metatype, type : RouteParamtypes.NEXT, data : null},
            transforms);
