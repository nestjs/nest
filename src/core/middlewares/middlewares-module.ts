...
import {
  ControllerMetadata
} from '@nestjs/common/interfaces/controllers/controller-metadata.interface';
import {Metatype} from '@nestjs/common/interfaces/metatype.interface';
import {
  MiddlewareConfiguration
} from
    '@nestjs/common/interfaces/middlewares/middleware-configuration.interface';
import {
  NestMiddleware
} from '@nestjs/common/interfaces/middlewares/nest-middleware.interface';
import {
  NestModule
} from '@nestjs/common/interfaces/modules/nest-module.interface';
import {isUndefined} from '@nestjs/common/utils/shared.utils';

import {
  InvalidMiddlewareException
} from '../errors/exceptions/invalid-middleware.exception';
...
export class MiddlewaresModule {
  ...
  public static async setup(container: NestContainer,
  ...
  public static getContainer(): MiddlewaresContainer { return this.container; }
  ...
  public static async setupMiddlewares(app) {
    ...
    await Promise.all(
        [...configs.entries() ].map(async ([ module, moduleConfigs ]) => {
          await Promise.all([...moduleConfigs ].map(
              async (config: MiddlewareConfiguration) => {
                await this.setupMiddlewareConfig(config, module, app);
              }));
        }));
  }

  public static async setupMiddlewareConfig(config: MiddlewareConfiguration,
                                            module: string, app) {
    const {forRoutes} = config;
    await Promise.all(forRoutes.map(
        async (route: ControllerMetadata&{method : RequestMethod}) => {
          await this.setupRouteMiddleware(route, config, module, app);
        }));
  }

  public static async setupRouteMiddleware(route: ControllerMetadata&
  ...
  private static async setupHandler(instance: NestMiddleware,
                                    metatype: Metatype<NestMiddleware>,
                                    app: any, method: RequestMethod,
                                    path: string) {
    ...
    const exceptionsHandler =
        this.routerExceptionFilter.create(instance, instance.resolve);
  ...
  private static setupHandlerWithProxy(exceptionsHandler: ExceptionsHandler,
                                       router: (...args) => void,
