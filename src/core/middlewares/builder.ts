...
import {
  BindResolveMiddlewareValues
} from '@nestjs/common/utils/bind-resolve-values.util';
import {
  isFunction,
  isNil,
  isUndefined
...
export class MiddlewareBuilder implements MiddlewaresConsumer {
  ...
  public build() { return [...this.middlewaresCollection ]; }

  private bindValuesToResolve(middlewares: Metatype<any>|Metatype<any>[],
                              resolveParams: any[]) {
  ...
  private static ConfigProxy = class implements MiddlewareConfigProxy {
    ...
    public forRoutes(...routes): MiddlewaresConsumer {
      const {middlewaresCollection, bindValuesToResolve, routesMapper} =
          this.builder;

      const forRoutes = this.mapRoutesToFlatList(routes.map(
          (route) => routesMapper.mapRouteToRouteProps(route),
          ));
      const configuration = {
