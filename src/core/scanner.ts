...
export class DependenciesScanner {
  ...
  public scanForModules(module: NestModuleMetatype,
  ...
  public reflectComponentMetadata(component: Metatype<Injectable>,
                                  token: string) {
  ...
  public reflectGatewaysMiddlewares(component: Metatype<Injectable>,
                                    token: string) {
  ...
  public reflectGuards(component: Metatype<Injectable>, token: string) {
    ...
    const flattenMethodsGuards =
        methodsGuards.reduce<any[]>((a: any[], b) => a.concat(b), []);
    [...controllerGuards, ...flattenMethodsGuards].map(
        (guard) => this.storeInjectable(guard, token));
  }

  public reflectInterceptors(component: Metatype<Injectable>, token: string) {
    const controllerInterceptors =
        this.reflectMetadata(component, INTERCEPTORS_METADATA);
    ...
    const flattenMethodsInterceptors =
        methodsInterceptors.reduce<any[]>((a: any[], b) => a.concat(b), []);
    [...controllerInterceptors, ...flattenMethodsInterceptors].map(
        (guard) => this.storeInjectable(guard, token));
  }

  public reflectKeyMetadata(component: Metatype<Injectable>, key: string,
                            method: string) {
    const descriptor =
        Reflect.getOwnPropertyDescriptor(component.prototype, method);
  ...
  public storeExportedComponent(exportedComponent: Metatype<Injectable>,
                                token: string) {
