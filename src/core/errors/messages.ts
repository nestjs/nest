...
export const InvalidModuleMessage = (scope: string) =>
    `Nest can't create the module instance. The frequent reason of this exception is the circular dependency between modules. Use forwardRef() to avoid it (read more https://docs.nestjs.com/advanced/circular-dependency). Scope [${
        scope}]`;

export const UnknownDependenciesMessage = (type: string) =>
    `Nest can't resolve dependencies of the ${
        type}. Please verify whether all of them are available in the current context.`;

export const UnknownExportMessage = (name: string) =>
    `You are trying to export unknown component (${
        name}). Remember - your component should be listed both in exports and components arrays!`;
...
export const INVALID_EXCEPTION_FILTER =
    `Invalid exception filters (@UseFilters()).`;
export const MICROSERVICES_PACKAGE_NOT_FOUND_EXCEPTION =
