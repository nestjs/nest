...
export class RouterExecutionContext {
  ...
  constructor(private readonly paramsFactory: IRouteParamsFactory,
  ...
  public create(instance: Controller, callback: (...args) => any,
                methodName: string, module: string,
    ...
    const isResponseObj =
        paramsMetadata.some(({type}) => type === RouteParamtypes.RESPONSE);
    ...
    return async (req, res, next) => {
      ...
      return !isResponseObj ? this.responseController.apply(
                                  result, res, requestMethod, httpCode)
                            : undefined;
    };
  }
  ...
  public reflectCallbackMetadata(instance: Controller,
                                 methodName: string): RouteParamsMetadata {
  ...
  public getArgumentsLength(keys: string[],
                            metadata: RouteParamsMetadata): number {
  ...
  public exchangeKeysForValues(keys: string[], metadata: RouteParamsMetadata):
      ParamProperties[] {
    return keys.map(key => {
      ...
      const extractValue = (req, res, next) =>
          this.paramsFactory.exchangeKeyForValue(type, data, {req, res, next});
  ...
  public mergeParamsMetatypes(
    ...
    return paramsProperties.map(
        (param) => ({...param, metatype : paramtypes[param.index]}));
  }

  public async getParamValue<T>(value: T, {metatype, type, data},
                                transforms: Transform<any>[]): Promise<any> {

    if (type === RouteParamtypes.BODY || type === RouteParamtypes.QUERY ||
        type === RouteParamtypes.PARAM) {

      return await this.pipesConsumer.apply(value, {metatype, type, data},
                                            transforms);
    }
