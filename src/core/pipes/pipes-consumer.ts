...
import {Paramtype, Transform} from '@nestjs/common/interfaces';
...
export class PipesConsumer {
  ...
  public async apply(value, {metatype, type, data},
                     transforms: Transform<any>[]) {
    const token = this.paramsTokenFactory.exchangeEnumForString(type);
    return await this.applyPipes(value, {metatype, type : token, data},
                                 transforms);
  }

  public async applyPipes(value,
