...
describe('WebSocketsController', () => {
  ...
  let provider: SocketServerProvider, config: ApplicationConfig,
  ...
  beforeEach(() => {
    ...
    instance = new WebSocketsController(
        provider, null, config, sinon.createStubInstance(WsContextCreator));
  });
  describe('hookGatewayIntoServer', () => {
    ...
    it('should throws "InvalidSocketPortException" when port is not a number',
       () => {
         expect(
             () => instance.hookGatewayIntoServer(new InvalidGateway(),
                                                  InvalidGateway, ''),
             )
    ...
    it('should call "subscribeObservableServer" with default values when metadata is empty',
       () => {
         ...
         expect(subscribeObservableServer.calledWith(gateway, '', 0))
             .to.be.true;
       });
    it('should call "subscribeObservableServer" when metadata is valid', () => {
      ...
      expect(subscribeObservableServer.calledWith(gateway, namespace, port))
          .to.be.true;
    });
  });
  describe('subscribeObservableServer', () => {
    let explorer: GatewayMetadataExplorer, mockExplorer: sinon.SinonMock,
        gateway, handlers, server, hookServerToProperties: sinon.SinonSpy,
  ...
  describe('subscribeEvents', () => {
    ...
    let handlers;
    let server, subscribeConnectionEvent: sinon.SinonSpy,
        subscribeDisconnectEvent: sinon.SinonSpy, nextSpy: sinon.SinonSpy,
        onSpy: sinon.SinonSpy, subscribeInitEvent: sinon.SinonSpy,
    ...
    it('should call "subscribeConnectionEvent" with expected arguments', () => {
      instance.subscribeEvents(gateway, handlers, server as any);
      expect(subscribeConnectionEvent.calledWith(gateway, server.connection))
          .to.be.true;
    });
    it('should call "subscribeDisconnectEvent" with expected arguments', () => {
      instance.subscribeEvents(gateway, handlers, server as any);
      expect(subscribeDisconnectEvent.calledWith(gateway, server.disconnect))
          .to.be.true;
    });
  ...
  describe('getConnectionHandler', () => {
    ...
    let handlers, fn;
    let connection, client, nextSpy: sinon.SinonSpy, onSpy: sinon.SinonSpy,
    ...
    beforeEach(() => {
      ...
      fn = instance.getConnectionHandler(instance, gateway, handlers, null,
                                         connection);
      fn(client);
    });

    it('should returns function', () => {
      expect(instance.getConnectionHandler(null, null, null, null, null))
          .to.be.a('function');
    });
    ...
    it('should call "subscribeMessages" with expected arguments', () => {
      expect(subscribeMessages.calledWith(handlers, client, gateway))
          .to.be.true;
    });
    ...
    describe('when defferedResult contains value which', () => {
      describe('is a Promise', () => {
        it('should returns Promise<Observable>', async () => {
          const value = 100;
          expect(await (await instance.pickResult(
      ...
      describe('is a value', () => {
        it('should returns Promise<Observable>', async () => {
          const value = 100;
          expect(await (await instance.pickResult(Promise.resolve(value)))
                     .toPromise())
              .to.be.eq(100);
        });
      });
