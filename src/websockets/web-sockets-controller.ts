...
export class WebSocketsController {
  ...
  public hookGatewayIntoServer(instance: NestGateway,
                               metatype: Metatype<Injectable>, module: string) {
  ...
  public subscribeObservableServer(instance: NestGateway, namespace: string,
                                   port: number, module: string) {
    ...
    const observableServer =
        this.socketServerProvider.scanForSocketServer(namespace, port);
  ...
  public subscribeEvents(
    ...
    const handler = this.getConnectionHandler(this, instance, messageHandlers,
  ...
  public getConnectionHandler(
    ...
    return (client) => {
      ...
      disconnectHook && disconnectHook.call(adapter, client,
                                            socket => disconnect.next(client));
    };
  }
  ...
  public subscribeMessages(messageHandlers: MessageMappingProperties[], client,
                           instance: NestGateway) {
    const adapter = this.config.getIoAdapter();
    const handlers =
        messageHandlers.map(({callback, message}) => ({
                              message,
  ...
  public async pickResult(defferedResult: Promise<any>):
