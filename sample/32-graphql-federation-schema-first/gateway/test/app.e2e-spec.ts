...
describe('GraphQL Federation Gateway (Schema First)', () => {
  ...
  it('should have schema generation scripts configured', () => {
    const packageJsonPath = join(__dirname, '..', 'package.json');
    const fs = require('fs');
    const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf-8'));

    // Check for required scripts
    expect(packageJson.scripts['generate:supergraph']).toBeDefined();
    expect(packageJson.scripts['generate:supergraph:rover']).toBeDefined();
    expect(packageJson.scripts['generate:typings']).toBeDefined();
  });
});
